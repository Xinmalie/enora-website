...
const STRIPE_PK = "pk_test_51TUtua3SPskkekQ6kCXo9UUpLpsrLIjWxHOzQ8m2MbKvQQZCoXC6KoOwVizInU3epE08jn24OM1K3zrjk5E4fup500CE2CwKDp";
...
// ─── PAYMENT MODAL ────────────────────────────────────────────────────────────
function PaymentModal({ item, onClose, onSuccess, T }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!stripeLib) {
        ...
        stripeLib = window.Stripe;
      }
      const stripe = stripeLib(STRIPE_PK);

      // Create payment intent via our serverless function
      const resp = await fetch("/api/create-payment-intent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items: [{ id: item.id }] }),
      });
      const { clientSecret, error: apiErr } = await resp.json();
      if (apiErr) throw new Error(apiErr);

      // Confirm payment with Stripe
      const { error: stripeErr } = await stripe.confirmCardPayment(clientSecret, {
        payment_method: {
          card: { number: cardNumber.replace(/\s/g,""), exp_month: expiry.split("/")[0], exp_year: "20"+expiry.split("/")[1], cvc },
      ...
      setDone(true);
      setTimeout(() => { onSuccess(item); }, 1500);
    } catch (e) {
  ...
  const inputStyle = {
    ...
    border: `1px solid ${T.subtle}`, fontSize: 14,
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: T.card, borderRadius: 16, padding: 32, width: "100%", maxWidth: 420,
        border: `1px solid ${T.orange}40`, boxShadow: `0 24px 64px ${T.orange}18`,
      }}>
        {done ? (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            <div style={{ fontSize: 56, marginBottom: 16 }}>🎉</div>
            <h3 style={{ color: T.white, fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Payment Successful!</h3>
            <p style={{ color: T.muted, fontSize: 14 }}>Your download is starting...</p>
          </div>
        ) : (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 11, letterSpacing: 3, color: T.orange, fontFamily: "monospace", fontWeight: 800, marginBottom: 6 }}>SECURE CHECKOUT</div>
                <h3 style={{ margin: 0, fontSize: 17, color: T.white, fontWeight: 800 }}>{item.title}</h3>
                <div style={{ color: T.orange, fontWeight: 900, fontSize: 22, marginTop: 4 }}>
                  {item.id === "bundle" ? "$2.50" : "$1.00"}
                  <span style={{ color: T.muted, fontSize: 13, fontWeight: 400, marginLeft: 8 }}>
                    {item.id === "bundle" ? "(B2 + C1 + C2)" : "USD"}
                  </span>
                </div>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input placeholder="Name on card" value={name} onChange={e => setName(e.target.value)} style={inputStyle} />
              <input placeholder="Card number" value={cardNumber} onChange={e => setCardNumber(formatCard(e.target.value))} style={inputStyle} maxLength={19} />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <input placeholder="MM/YY" value={expiry} onChange={e => setExpiry(formatExp(e.target.value))} style={inputStyle} maxLength={5} />
                <input placeholder="CVC" value={cvc} onChange={e => setCvc(e.target.value.replace(/\D/g,"").slice(0,4))} style={inputStyle} maxLength={4} />
              </div>
            </div>

            {error && <p style={{ color: T.magenta, fontSize: 13, margin: "10px 0 0" }}>{error}</p>}

            <button onClick={handlePay} disabled={loading} style={{
              width: "100%", marginTop: 16, padding: "14px",
              background: loading ? T.subtle : T.orange,
              ...
              {loading ? "Processing..." : `Pay ${item.id === "bundle" ? "$2.50" : "$1.00"} →`}
            </button>

            <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "center", marginTop: 14 }}>
              <span style={{ fontSize: 12, color: T.muted }}>🔒 Secured by Stripe</span>
              <span style={{ color: T.subtle }}>·</span>
              <span style={{ fontSize: 12, color: T.muted }}>Your card info is never stored</span>
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <p style={{ color: T.muted, marginBottom: 28, fontSize: 15 }}>
        Full worksheets covering grammar, vocabulary, reading, writing, and speaking — one PDF per level.
      </p>

      {/* Free */}
      <SLabel T={T} color={T.teal}>Free Worksheets</SLabel>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(265px,1fr))", gap: 12, marginBottom: 36 }}>
        {RESOURCES.filter(r => r.free).map(r => {
          const ck = levelColorKey[r.level] || "teal";
          return (
            <Card key={r.title} style={{ padding: 22 }} accent={T[ck]} T={T}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                <div style={{ fontSize: 26, color: T[ck] }}>📄</div>
                <div style={{ flex: 1 }}>
      ...
      {/* Bundle */}
      <Card style={{ padding: 24, marginBottom: 24, border: `1px solid ${T.orange}40`, background: T.orange + "08" }} hover={false} T={T}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
          <div>
            <div style={{ fontSize: 11, letterSpacing: 3, color: T.orange, fontFamily: "monospace", fontWeight: 800, marginBottom: 6 }}>BEST VALUE</div>
            <h3 style={{ margin: "0 0 4px", fontSize: 18, color: T.white, fontWeight: 800 }}>Premium Bundle — B2 + C1 + C2</h3>
            <p style={{ margin: 0, color: T.muted, fontSize: 13 }}>All 3 advanced worksheets in one purchase. Save $0.50 vs buying separately.</p>
          </div>
            ...
            <Card key={r.title} style={{ padding: 22 }} accent={T[ck]} T={T}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, color: T.white, fontSize: 14, marginBottom: 6 }}>{r.title}</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <Badge text="PDF" color={T[ck]} />
                    <Badge text={r.level} color={T[ck]} />
                    <Badge text="$1.00" color={T.orange} />
                  </div>
                </div>
              </div>
              {r.stripeLink
      ...
      <div style={{ padding: "16px 20px", border: `1px solid ${T.cardBorder}`, borderRadius: 10, background: T.surface }}>
        <p style={{ margin: 0, color: T.muted, fontSize: 13, fontFamily: "monospace" }}>
          🔒 Payments secured by Stripe · Instant download after payment · No account required
        </p>
      </div>
    </div>
  );
